...
from ..core.urls import normalize_url, get_domain, is_html_content
from ..core.http import create_permissive_ssl_context
...
class Crawler:
    ...
    def __init__(
        ...
        # Extractors are created through the registry so this class never
        # names PDFExtractor / WordExtractor / … directly.
        self._extractors = None
        if self.extract_docs:
            from ..extract.registry import create_registry
            self._extractors = create_registry(
                timeout=timeout,
        ...
        self.parser = parser
        self._processor = PageProcessor(self.pages_dir, save_html=save_html, parser=parser)
    ...
    def _process_document(self, url: str, depth: int) -> Optional[List[Tuple[str, int]]]:
        ...
        if path.endswith('.pdf'):
            result = self._extractors.get('pdf').extract_from_url(url)
    ...
    def _process_pdf_content(self, url: str, content_bytes: bytes, depth: int) -> Optional[List[Tuple[str, int]]]:
        ...
        result = self._extractors.get('pdf').extract_from_bytes(content_bytes)
    ...
    def _extract_docx(self, url: str, tmp_path: 'Path', depth: int) -> List[Tuple[str, int]]:
        ...
        import os
        try:
            result = self._extractors.get('docx').extract(tmp_path)
    ...
    def _extract_xlsx(self, url: str, tmp_path: 'Path', depth: int) -> List[Tuple[str, int]]:
        ...
        import os
        try:
            documents = self._extractors.get('xlsx').extract(tmp_path)
            if not documents:
    ...
    def _extract_pptx(self, url: str, tmp_path: 'Path', depth: int) -> List[Tuple[str, int]]:
        ...
        import os
        try:
            documents = self._extractors.get('pptx').extract_combined(tmp_path)
